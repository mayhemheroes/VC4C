...
# ---------------------------------------------------------------------------
# 3b. Compile the file-input harness for the `vc4c` Mayhem target.
#
#    Mayhem's `vc4c` target is a FILE-input target (cmd: /mayhem/vc4c_target @@).
#    It reads the input file and drives VC4C's built-in SPIR-V front-end + full
#    compilation pipeline WITH exception handling -- so malformed / empty input
#    exits cleanly instead of the std::terminate()/SIGABRT the real vc4c CLI does
#    (which Mayhem flags as an "Uncaught Exception" critical error -> 0 edges).
#    Same sanitized flags as the CLI so Mayhem instruments/traces it identically;
#    /tmp/vc4c_asan.o bakes detect_leaks=0 for Mayhem's ptrace tracer.
# ---------------------------------------------------------------------------
$CXX $SANITIZER_FLAGS $DEBUG_FLAGS \
    -std=c++14 \
    $INCLUDES \
    "$SRC/mayhem/vc4c_target.cpp" \
    /tmp/vc4c_asan.o \
    -L"$SRC/build/src" -lVC4CC \
    -Wl,-rpath,/mayhem/build/src \
    -o /mayhem/vc4c_target

[ -f /mayhem/vc4c_target ] || { echo "ERROR: /mayhem/vc4c_target not built" >&2; exit 1; }
...
echo "build.sh: OK — /mayhem/fuzz_spirv, /mayhem/fuzz_spirv-standalone, /mayhem/vc4c_target built"
